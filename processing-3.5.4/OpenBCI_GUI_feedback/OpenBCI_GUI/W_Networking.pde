...
//    Updated by: Aya Elsayed, May 2020. Adding receiving and sending simultenous and reporting info in a text file
///////////////////////////////////////////////////////////////////////////////

///////////////////////////////////////////////////////////////for naming the text file
...
class W_Networking extends Widget {
    ...
    void stopNetwork() {
        ...
        if (stream4!=null) {
          ...
          Nresults.flush(); // Writes the remaining data to the file
          Nresults.close(); // Finishes the file
    }
...
class Stream extends Thread {
    ...
    LSL.StreamInlet inlet;// inlet for receiving stream
    LSL.StreamInfo[] results;// receiving stream in results
                    ...
                    try {
                        ...
                        if (checkForData()) {
                            if (this.dataType.equals("TimeSeries")) {
                                //sendTimeSeriesData();
                                ReceiveData();
                            } else if (this.dataType.equals("Receive"))  {
                               ReceiveData();
    ...
    void sendTimeSeriesData() {
        ...
        if (filter==0) {
            ...
            if (this.protocol.equals("OSC")) {
                ...
                println("sending timeseries lsl");
                // SERIAL
    ...
    void ReceiveData() {
      //println("receiving mode");
      long Rtime =  System.currentTimeMillis();
                   try {
               dataToreceive= new float[inlet.info().channel_count()];
               //println("stuck in oping");
                  }
            catch(Exception e) {
          ...
          if (this.protocol.equals("LSL")) {
                 print("-- FPS: " + frameRate + " --          ");
                 Nresults.print("-- FPS: " + frameRate + " --          "); //Write in the text file
                 
               try {
                 // NB: blocking call, FPS will drop to stream's sample rate
                inlet.pull_sample(dataToreceive); //pulling a sample 
                Rtime =  System.currentTimeMillis(); //time of receiving
                println("time stamp of receiving ",Rtime);
              ...
              received_Data = int(dataToreceive[1]); //Save the second element of the receiving frame in received_Data global variable
              
       for (int k=0; k<dataToreceive.length; k++)
        {print(" received data \t" + Double.toString(dataToreceive[k]));
        Nresults.print(" received data \t" + Double.toString(dataToreceive[k])); // write the received frame in the text file, if any thing changed in the print it will effect the extraction_info python script
        }             
                if (dataToreceive[0]==-1)       // in case of receiving "-1" flag
              {println("***************************");
              Nresults.println("***************************"); // important for the extraction_info python script
                }
               println();
               
                Nresults.print("time stamp of receiving ");
                Nresults.println( Rtime);
    }          
    ...
    void sendPowerBandData() {
        ...
        if (this.filter==0 || this.filter==1) {
            ...
            // UDP
                ...
                /////////////////counter to send a "-1" flag for the calculating the round-trip delay
                if (temp == 5)
                { dataToSend[19] = -1; // "-1" flag 
                  temp = 0;
                }
                else {temp++;}
                ///// printing the power band & writting the band power in the text file
                print( "\n"+"power band ",dataToSend[0] ,dataToSend[1] ,dataToSend[2] ,dataToSend[3]  ,dataToSend[4]  ,dataToSend[5]  ,dataToSend[6]  ,dataToSend[7] ,dataToSend[8]  );
                println(dataToSend[9]  ,dataToSend[10] ,dataToSend[11] ,dataToSend[12] ,dataToSend[13]  ,dataToSend[14]  ,dataToSend[15]  ,dataToSend[16]  ,dataToSend[17] ,dataToSend[18]  ,dataToSend[19]  , System.currentTimeMillis());
                Nresults.print("\n"+"power band ");Nresults.print(dataToSend[0]+ " "  ); Nresults.print(dataToSend[1]+ " "); Nresults.print(dataToSend[2]+ " "); Nresults.print(dataToSend[3]+ " "); Nresults.print(dataToSend[4]+ " "); Nresults.print(dataToSend[5]+ " "); Nresults.print(dataToSend[6]+ " ");
                Nresults.print(dataToSend[7]+ " "); Nresults.print(dataToSend[8]+ " "  );Nresults.print(dataToSend[9]+ " ");
                Nresults.print(dataToSend[10]+ " "); Nresults.print(dataToSend[11]+ " " );Nresults.print(dataToSend[12]+ " ");Nresults.print(dataToSend[13]+ " "); Nresults.print(dataToSend[14]+ " "); Nresults.print(dataToSend[15]+ " "); Nresults.print(dataToSend[16]+ " ");
                Nresults.print(dataToSend[17]+ " "); Nresults.print(dataToSend[18] +" " ); Nresults.print(dataToSend[19]+ " "); Nresults.print("\n time stamp of sending "); Nresults.println(System.currentTimeMillis());
                
                // Add timestamp to LSL Stream
                outlet_data.push_chunk(dataToSend, System.currentTimeMillis());
                println("time stamp ", System.currentTimeMillis());
    ...
    void openNetwork() {
        ...
        if (this.protocol.equals("OSC")) {
           ...
           // sending setup
            ...
            println(this.streamType);
            //receiving setup
             println("Resolving an EEG stream...");//aya**
           // NB: blocking until at least on stream is found
           results = LSL.resolve_stream("source_id","sdfwerr32432");//"source_id","sdfwerr32432" //for receiving from matlab
