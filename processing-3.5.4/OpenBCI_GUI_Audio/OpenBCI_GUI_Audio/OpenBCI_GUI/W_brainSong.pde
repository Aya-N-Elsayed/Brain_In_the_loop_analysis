...
//
//    This is a widget intended purely for fun and/or entertainment.  It plays a "song" with up to 5 simultaneous notes
//    based on the brain response at individual electrodes in five fundemental bands (theta, delta, alpha, beta, gamma).
//    The notes are selected based on which electrode has the most power in a given band using lookup tables.  The
//    electrode-to-note mapping is mostly arbitrary and can be easily changed.  The volume of each of the mixed tones is
//    proportional to the overall power in the frequency band corresponding to that tone.
//
//    In addition to playing the "song", this widget also shows a related headplot.  Electrodes on the headplot that are lit
//    up correspond to electrodes that were selected in that time instant and whose notes are thus being played.  The color
//    of the lit up electrode indicates which frequency band led to its selection: red = theta, green = delta, blue = alpha,
//    cyan = beta, and magenta = gamma. If an electrode has the highest power in more than one band, the color used will be
//    the one corresponding to the band with the higher overall power.
//    Written by Chuck Creusere, Feb. 2019
//    Based on headPlot code written by Conor Russomanno, November 2016
//    Based on code written by: Chip Audette, Oct 2013
//
///////////////////////////////////////////////////,


// Duplicates of globals from widget headPlot
...
// Global variables for headplot parameters
int active_electrode[] = new int[5];
float rel_power[] = new float[5];
 
class W_brainSong extends Widget {

  //to see all core variables/methods of the Widget class, refer to Widget.pde
  //put your custom variables here...

  HeadPlotBS headPlotBS;

  W_brainSong(PApplet _parent){
    super(_parent); //calls the parent CONSTRUCTOR method of Widget (DON'T REMOVE)

    //This is the protocol for setting up dropdowns.
    //Note that these 3 dropdowns correspond to the 3 global functions below
 
    addDropdown("ContrSound", "Sound", Arrays.asList("On","Off"), 1);
    addDropdown("MaxVoice", "Max Voices", Arrays.asList("5", "4", "3", "2", "1"), 0);
    //Initialize the headplot
    updateHeadPlotBS(nchan);
...
//   Initialize oscillators
  ...
  void updateHeadPlotBS(int _nchan) {
    //add your code here
    headPlotBS = new HeadPlotBS(x, y, w, h, win_x, win_y);
    
    //FROM old Gui_Manager
    headPlotBS.setIntensityData_byRef(dataProcessing.data_std_uV, is_railed);
    headPlotBS.setPolarityData_byRef(dataProcessing.polarity);
    headPlotBS.drawHeadAsContours = false;
...
// Set voice amplitude based on the relative band power and maximum number of voices allowed 

    Arrays.sort(idx, new Comparator<Integer>() {
      @Override public int compare(final Integer o1, final Integer o2) {
        return Float.compare(rel_power[o1], rel_power[o2]);
      }
    });
   ...
   wave5.setAmplitude(0);
 
   for(k=5;k>5-voices_output;k--) {
     switch (idx[k-1]) {
       case 0:
         wave3.setAmplitude(rel_power[0]);
         break;
       case 1:
         wave2.setAmplitude(rel_power[1]); 
         break;
       case 2:
         wave1.setAmplitude(rel_power[2]);
         break;
       case 3:
         wave4.setAmplitude(rel_power[3]);
         break;
       case 4:
         wave5.setAmplitude(rel_power[4]);
         break;
     }
   }
...
// Select note based on electrode with most theta power
 ...
 // Select note based on electrode with most gamma power
  ...
  void draw(){
    ...
    //put your code here
    headPlotBS.draw(); //draw the actual headplot
  ...
  void screenResized(){
    ...
    headPlotBS.hp_w = w;
    headPlotBS.hp_h = h;
    headPlotBS.hp_win_x = x;
    headPlotBS.hp_win_y = y;

    thread("doHardCalcsBS");
...
void setVertScaleFactor_indBS(int ind) {
  vertScaleFactor_ind = max(0,ind);
  if (ind >= vertScaleFactor.length) vertScaleFactor_ind = 0;
  updateVertScaleBS();
}

void updateVertScaleBS() {
  vertScale_uV = default_vertScale_uV * vertScaleFactor[vertScaleFactor_ind];
  w_brainSong.headPlotBS.setMaxIntensity_uV(vertScale_uV);
}

void doHardCalcsBS() {
  if (!w_brainSong.headPlotBS.threadLock) {
    w_brainSong.headPlotBS.threadLock = true;
    w_brainSong.headPlotBS.setPositionSize(w_brainSong.headPlotBS.hp_x, w_brainSong.headPlotBS.hp_y, w_brainSong.headPlotBS.hp_w, w_brainSong.headPlotBS.hp_h, w_brainSong.headPlotBS.hp_win_x, w_brainSong.headPlotBS.hp_win_y);
    w_brainSong.headPlotBS.hardCalcsDone = true;
    w_brainSong.headPlotBS.threadLock = false;
  }
}
...
class HeadPlotBS {
  ...
  HeadPlotBS(int _x, int _y, int _w, int _h, int _win_x, int _win_y) {
  ...
  private void updateElectrodeColors() {
    final Integer[] idx = {0, 1, 2, 3, 4};
    int[] red_comp = {255, 0, 0, 255, 0};
    int[] green_comp = {0, 255, 0, 0, 255}; 
    int[] blue_comp = {0, 0, 255, 255, 255};    
    
 //   System.out.println("Number of electrodes: " + electrode_xy.length);

// Sort frequency bands by relative power.  This will be used to determine what color is
// shown on the electrode in the event that that electrode has the top power in more than one band.  The 
// color shown on the electrode will correspond to the band with the most power.
    ...
    for (int Ielec=0; Ielec < electrode_xy.length; Ielec++) {
      electrode_rgb[0][Ielec] = 255;
      electrode_rgb[1][Ielec] = 255;
      electrode_rgb[2][Ielec] = 255;
    }

    for (int k=5-voices_output;k<5;k++) {
      electrode_rgb[0][active_electrode[idx[k]]] = red_comp[idx[k]];
      electrode_rgb[1][active_electrode[idx[k]]] = green_comp[idx[k]];
      electrode_rgb[2][active_electrode[idx[k]]] = blue_comp[idx[k]];
    }
