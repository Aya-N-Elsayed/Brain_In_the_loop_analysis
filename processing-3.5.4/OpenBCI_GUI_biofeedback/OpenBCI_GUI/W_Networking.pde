...
class W_Networking extends Widget {
    ...
    void startNetwork() {
        ...
        //Nresults = createWriter("Nresults.txt");
    }
...
class Stream extends Thread {
    ...
    LSL.StreamInlet inlet;//aya
    LSL.StreamInfo[] results;//aya
                ...
                if (!isRunning) {
                    try {
                        ...
                        if (checkForData()) {
                            if (this.dataType.equals("TimeSeries")) {
                                //sendTimeSeriesData();
                                recieveTimeSeriesData();
                            } else if (this.dataType.equals("Recieve"))  {
                               recieveTimeSeriesData();
    ...
    ///////////////////////////////////aya recieving data from lsl
    void recieveTimeSeriesData() {
      //println("recieving mode");
      long Rtime =  System.currentTimeMillis();
                   try {
               dataTorecieve= new float[inlet.info().channel_count()];
        ...
        if (filter==0) {
          if (this.protocol.equals("LSL")) {
                 print("-- FPS: " + frameRate + " --          ");
                 Nresults.print("-- FPS: " + frameRate + " --          ");
                 
               try {
                 // NB: blocking call, FPS will drop to stream's sample rate
                inlet.pull_sample(dataTorecieve);
                Rtime =  System.currentTimeMillis();
                println("time stamp of recieving ",Rtime);
              ...
              recieved_Data = int(dataTorecieve[1]);
              //println("1_recieved ",recieved_Data);
              //println("2_recieved",dataTorecieve[0]);
              
       for (int k=0; k<dataTorecieve.length; k++)
        {print(" recieved data \t" + Double.toString(dataTorecieve[k]));
        Nresults.print(" recieved data \t" + Double.toString(dataTorecieve[k]));
        }
        //print(System.currentTimeMillis());
              
                if (dataTorecieve[0]==-1)
              {println("***************************");
              Nresults.println("***************************");
                }
               println();
               
                Nresults.print("time stamp of recieving ");
    ...
    void openNetwork() {
        ...
        if (this.protocol.equals("OSC")) {
           ...
           results = LSL.resolve_stream("source_id","sdfwerr32432");//aya //changed10 ,"source_id","sdfwerr32432" //receiving
           println("Number of streams found: " + results.length);//aya
              // open an inlet
            inlet = new LSL.StreamInlet(results[0]);//aya receiving
            Nresults = createWriter("Nresults.txt");
